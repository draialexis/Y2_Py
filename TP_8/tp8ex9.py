...
should_be_1 = int_rect_g(lambda x: np.sin(x), 0, np.pi / 2, 100)
print(should_be_1)
...
should_be_1 = int_rect_d(lambda x: np.sin(x), 0, np.pi / 2, 100)
print(should_be_1)
...
for i in range(7):
    n *= 10
    area = int_rect_g(lambda x: pow(np.e, np.sin(x)), -1 * np.pi, np.pi, n)
...
for i in range(7):
    n *= 10
    area = int_rect_d(lambda x: np.e * (-1 * pow(x, 2)), -10, 2, n)
...
print("fun_1 integral:", quad(lambda x: pow(np.e, np.sin(x)), -1 * np.pi, np.pi))
print("fun_2 integral:", quad(lambda x: np.e * (-1 * pow(x, 2)), -10, 2))
