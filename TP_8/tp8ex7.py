...
# putting b around pi seems like it'll work

print('\n=============2)=============\n')

# appropriate intervals could be [-2, 0] and [0, 2]
print("{x | f(x) = 0} = ", dicho(lambda x: 2 * x + 3 - pow(np.e, x), -2, 0, epsilon))
print("{x | f(x) = 0} = ", dicho(lambda x: 2 * x + 3 - pow(np.e, x), 1, 1400, epsilon))
